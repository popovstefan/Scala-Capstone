...
object Interaction2 extends Interaction2Interface {
  ...
  private val temperatureToColor = List[(Temperature, Color)](
  ...
  private val deviationToColor = List[(Temperature, Color)](
    (7, Color(0, 0, 0)),
    (4, Color(255, 0, 0)),
    (2, Color(255, 255, 0)),
    (0, Color(255, 255, 255)),
  ...
  def availableLayers: Seq[Layer] = List(
    Layer(layerName = LayerName.Temperatures, colorScale = temperatureToColor, bounds = Range(1975, 2015)),
    Layer(layerName = LayerName.Deviations, colorScale = deviationToColor, bounds = Range(1990, 2015))
  )
  ...
  def layerUrlPattern(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = Signal(
    "target/" + selectedLayer().layerName.id + "/" + selectedYear() + "/{z}/{x}-{y}.png'"
  )
  ...
  def caption(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = Signal(
    selectedLayer().layerName.id + " (" + selectedYear() + ")"
  )
